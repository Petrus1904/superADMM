 ...
 * The int array Ai and the CSfloat array Ax are of size nz.  This data structure
 ...
 * CSfloat array Lx of the same size as Li).  It has a unit diagonal, which is
...
(
    ...
    LDL_int Ap [ ],	/* input of size n+1, not modified */
    LDL_int Ai [ ],	/* input of size nz=Ap[n], not modified */
    CSfloat Ax [ ],	/* input of size nz=Ap[n], not modified */
    ...
    CSfloat Lx [ ],	/* output of size lnz=Lp[n], not defined on input */
    CSfloat D [ ],	/* output of size n, not defined on input */
    CSfloat Y [ ],	/* workspace of size n, not defn. on input or output */
...
)
{
    ...
    for (k = 0 ; k < n ; k++)
    {
        ...
        if (D [k] == 0.0) return (-k-1) ; /* failure, D(k,k) is zero, offset with -1 to ensure negativity */
        if(D[k] > 0) posCnt++;
    }
    return (posCnt) ;	/* success, diagonal of D is all nonzero */
}
...
(
    ...
    CSfloat X [ ],	/* size n.  right-hand-side on input, soln. on output */
    LDL_int Lp [ ],	/* input of size n+1, not modified */
    LDL_int Li [ ],	/* input of size lnz=Lp[n], not modified */
    CSfloat Lx [ ]	/* input of size lnz=Lp[n], not modified */
)
{
    LDL_int j, p, p2 ;
    CSfloat xj;
...
(
    ...
    CSfloat X [ ],	/* size n.  right-hand-side on input, soln. on output */
    CSfloat D [ ]	/* input of size n, not modified */
)
{
    LDL_int j ;
    for (j = 0 ; j < n ; j++)
...
(
    ...
    CSfloat X [ ],	/* output of size n. */
    CSfloat B [ ],	/* input of size n. */
