...
*/
{
    CSint p, j, nz = 0, anz, *Cp, *Ci, *Bp, m, n, bnz, *w, values;
    CSfloat *x, *Bx, *Cx;
    cs *C;
    if (!A || !B) return (NULL);	/* check inputs */
    m = A->m; anz = A->p[A->n];
    n = B->n; Bp = B->p; Bx = B->x; bnz = Bp[n];
    w = cs_calloc (m, sizeof (CSint));
    values = (A->x != NULL) && (Bx != NULL);
    x = values ? cs_malloc (m, sizeof (CSfloat)) : NULL;
...
static CSint cs_wclear (CSint mark, CSint lemax, CSint *w, CSint n)
/*
  Purpose:
...
static CSint cs_diag (CSint i, CSint j, CSfloat aij, void *other) 
{ 
  return (i != j); 
}
...
{
    cs *C, *A2, *AT;
    CSint *Cp, *Ci, *last, *ww, *len, *nv, *next, *P, *head, *elen, *degree, *w,
...
static
CSint cs_ereach (const cs *A, CSint k, const CSint *parent, CSint *s, CSint *w,
    CSfloat *x, CSint top)
{
    CSint i, p, len, *Ap = A->p, *Ai = A->i;
    CSfloat *Ax = A->x;
...
{
    CSfloat d, lki, *Lx, *x;
    CSint top, i, p, k, n, *Li, *Lp, *cp, *Pinv, *w, *s, *c, *parent;
    cs *L, *C, *E;
    csn *N;
    if (!A || !S || !S->cp || !S->parent) return (NULL);   /* check inputs */
    n = A->n;
    N = cs_calloc (1, sizeof (csn));
    w = cs_malloc (3*n, sizeof (CSint)); s = w + n, c = w + 2*n;
    x = cs_malloc (n, sizeof (CSfloat));
...
static void cs_cedge (CSint j, CSint i, const CSint *first, CSint *maxfirst, CSint *delta,
    CSint *prevleaf, CSint *ancestor)
{
    CSint q, s, sparent, jprev;
...
{
    CSint i, j, k, p, n, m, ii, s, *ATp, *ATi, *maxfirst, *prevleaf, *ancestor,
	*head = NULL, *next = NULL, *colcount, *w, *first, *delta;
    cs *AT;
    if (!A || !parent || !post) return (NULL);	    /* check inputs */
    m = A->m; n = A->n;
    s = 4*n + (ata ? (n+m+1) : 0);
    w = cs_malloc (s, sizeof (CSint)); first = w+3*n;	/* get workspace */
    ancestor = w; maxfirst = w+n; prevleaf = w+2*n;
    delta = colcount = cs_malloc (n, sizeof (CSint));	/* allocate result */
    AT = cs_transpose (A, 0);
...
{
    CSint i, p, p2, done, jnew, head = 0, *Lp, *Li;
...
static CSint cs_bfs (const cs *A, CSint n, CSint *wi, CSint *wj, CSint *queue,
    const CSint *imatch, const CSint *jmatch, CSint mark)
{
    CSint *Ap, *Ai, head = 0, tail = 0, j, i, p, j2;
    cs *C;
...
static void cs_matched (CSint m, const CSint *wi, const CSint *jmatch, CSint *P, CSint *Q,
    CSint *cc, CSint *rr, CSint set, CSint mark)
{
    CSint kc = cc[set], i;
    CSint kr = rr[set-1];
    for (i = 0; i < m; i++)
...
static void cs_unmatched (CSint m, const CSint *wi, CSint *P, CSint *rr, CSint set)
/* 
  Purpose:
...
static CSint cs_rprune (CSint i, CSint j, CSfloat aij, void *other)
{
    CSint *rr = (CSint *) other;
...
{
    CSint m, n, i, j, k, p, cnz, nc, *jmatch, *imatch, *wi, *wj, *Pinv, *Cp, *Ci,
...
static CSint cs_tol (CSint i, CSint j, CSfloat aij, void *tol)
{
    return (fabs (aij) > *((CSfloat *) tol));
}
...
static CSint cs_nonzero (CSint i, CSint j, CSfloat aij, void *other)
{
    return (aij != 0);
}
...
*/
{
    CSint i, j, p, q, nz = 0, n, m, *Ap, *Ai, *w;
    CSfloat *Ax;
    if (!A) return (0);			/* check inputs */
    m = A->m; n = A->n; Ap = A->p; Ai = A->i; Ax = A->x;
    w = cs_malloc (m, sizeof (CSint));		/* get workspace */
...
{
    CSint i, k, p, m, n, inext, *Ap, *Ai, *w, *parent, *ancestor, *prev;
    if (!A) return (NULL);		    /* check inputs */
    m = A->m; n = A->n; Ap = A->p; Ai = A->i;
    parent = cs_malloc (n, sizeof (CSint));
    w = cs_malloc (n + (ata ? m : 0), sizeof (CSint));
    ancestor = w; prev = w + n;
...
{
    void *p2;
    *ok = !CS_OVERFLOW (n,size);	    /* guard against CSint overflow */
...
static void cs_augment (CSint k, const cs *A, CSint *jmatch, CSint *cheap, CSint *w,
	CSint *js, CSint *is, CSint *ps)
{
    CSint found = 0, p, i = -1, *Ap = A->p, *Ai = A->i, head = 0, j;
...
CSint *cs_maxtrans (const cs *A)   /* returns jmatch[0..m-1]; imatch[0..n-1] */
{
    CSint i, j, k, n, m, p, n2 = 0, m2 = 0, *Ap, *jimatch, *w, *cheap, *js, *is,
	*ps, *Ai, *Cp, *jmatch, *imatch;
    cs *C;
    if (!A) return (NULL);			    /* check inputs */
    n = A->n; m = A->m; Ap = A->p; Ai = A->i;
    w = jimatch = cs_calloc (m+n, sizeof (CSint));   /* allocate result */
    ...
    w = cs_malloc (5*n, sizeof (CSint));		    /* allocate workspace */
...
{
    CSint p, j, nz = 0, anz, *Cp, *Ci, *Bp, m, n, bnz, *w, values, *Bi;
    CSfloat *x, *Bx, *Cx;
    cs *C;
    if (!A || !B) return (NULL);	/* check inputs */
    m = A->m; anz = A->p[A->n];
...
cs *cs_permute (const cs *A, const CSint *Pinv, const CSint *Q, CSint values)
{
    CSint p, j, k, nz = 0, m, n, *Ap, *Ai, *Cp, *Ci;
    CSfloat *Cx, *Ax;
    cs *C;
    if (!A) return (NULL);		/* check inputs */
    m = A->m; n = A->n; Ap = A->p; Ai = A->i; Ax = A->x;
...
/* Pinv = P', or P = Pinv' */
CSint *cs_pinv (CSint const *P, CSint n)
{
    CSint k, *Pinv;
    if (!P) return (NULL);			/* P = NULL denotes identity */
    Pinv = cs_malloc (n, sizeof (CSint));	/* allocate resuult */
...
{
    CSint j, k = 0, *post, *w, *head, *next, *stack;
    if (!parent) return (NULL);			/* check inputs */
    post = cs_malloc (n, sizeof (CSint));		/* allocate result */
    w = cs_malloc (3*n, sizeof (CSint));			/* 3*n workspace */
...
{
    CSfloat *Rx, *Vx, *Ax, *Beta, *x;
    CSint i, k, p, m, n, vnz, p1, top, m2, len, col, rnz, *s, *leftmost, *Ap,
	*Ai, *parent, *Rp, *Ri, *Vp, *Vi, *w, *Pinv, *Q;
    ...
    vnz = S->lnz; rnz = S->unz;
    leftmost = Pinv + m + n;
    w = cs_malloc (m2+n, sizeof (CSint));
    x = cs_malloc (m2, sizeof (CSfloat));
    N = cs_calloc (1, sizeof (csn));
    ...
    N->B = Beta = cs_malloc (n, sizeof (CSfloat));
...
CSint cs_qrsol (const cs *A, CSfloat *b, CSint order)
{
	...
	x = cs_calloc (S ? S->m2 : 1, sizeof (CSfloat));
	ok = (S && N && x);
	if (ok)
	{
...
{
    CSint n, i, k, b = 0, top, *xi, *pstack, *P, *R, *Ap, *ATp;
    cs *AT;
    csd *D;
    if (!A) return (NULL);
    n = A->n; Ap = A->p;
    ...
    xi = cs_malloc (2*n, sizeof (CSint));    /* allocate workspace */
    pstack = xi + n;
...
css *cs_schol (const cs *A, CSint order)
{
    ...
    cs_free (post);
    cs_spfree (C);
    S->cp = cs_malloc (n+1, sizeof (CSint)); /* find column pointers for L */
...
CSint cs_splsolve (cs *L, const cs *B, CSint k, CSint *xi, CSfloat *x, const CSint *Pinv)
{
    CSint j, jnew, p, px, top, n, *Lp, *Li, *Bp, *Bi;
    CSfloat *Lx, *Bx;
...
static CSint *cs_vcount (const cs *A, const CSint *parent, CSint *m2, CSint *vnz)
{
    CSint i, k, p, pa, n = A->n, m = A->m, *Ap = A->p, *Ai = A->i;
    CSint *Pinv = cs_malloc (2*m+n, sizeof (CSint)), *leftmost = Pinv + m + n;
    CSint *w = cs_malloc (m+3*n, sizeof (CSint));
    CSint *next = w, *head = w + m, *tail = w + m + n, *nque = w + m + 2*n;
...
{
    CSint i, j, p, q, i2, j2, n, *Ap, *Ai, *Cp, *Ci, *w;
    CSfloat *Cx, *Ax;
    cs *C;
    if (!A) return (NULL);
    n = A->n; Ap = A->p; Ai = A->i; Ax = A->x;
    C = cs_spalloc (n, n, Ap[n], values && (Ax != NULL), 0);
    w = cs_calloc (n, sizeof (CSint));
...
CSint cs_tdfs (CSint j, CSint k, CSint *head, const CSint *next, CSint *post, CSint *stack)
{
    CSint i, p, top = 0;
...
{
    CSint m, n, nz, p, k, *Cp, *Ci, *w, *Ti, *Tj;
    CSfloat *Cx, *Tx;
    cs *C;
    if (!T) return (NULL);				/* check inputs */
    m = T->m; n = T->n; Ti = T->i; Tj = T->p; Tx = T->x; nz = T->nz;
    C = cs_spalloc (m, n, nz, Tx != NULL, 0);		/* allocate result */
    w = cs_calloc (n, sizeof (CSint));			/* get workspace */
    if (!C || !w) return (cs_done (C, w, NULL, 0));	/* out of memory */
    Cp = C->p; Ci = C->i; Cx = C->x;
...
{
    CSint p, f, j, *Lp, *Li, *Cp, *Ci, n;
    CSfloat *Lx, *Cx, alpha, beta = 1, delta, gamma, w1, w2, *w, beta2 = 1;
    if (!L || !C || !parent) return (0);
    Lp = L->p; Li = L->i; Lx = L->x; n = L->n;
    Cp = C->p; Ci = C->i; Cx = C->x;
    if ((p = Cp[0]) >= Cp[1]) return (1);	    /* return if C empty */
    w = cs_malloc (n, sizeof (CSfloat));
    if (!w) return (0);
    f = Ci[p];
...
cs *cs_spalloc (CSint m, CSint n, CSint nzmax, CSint values, CSint triplet)
{
    ...
    A->p = cs_malloc (triplet ? nzmax : n+1, sizeof (CSint));
    A->i = cs_malloc (nzmax, sizeof (CSint));
    A->x = values ? cs_malloc (nzmax, sizeof (CSfloat)) : NULL;
...
{
    CSint ok, oki, okj = 1, okx = 1;
    if (!A) return (0);
    nzmax = (nzmax <= 0) ? (A->p[A->n]) : nzmax;
    A->i = cs_realloc (A->i, nzmax, sizeof (CSint), &oki);
    if (A->nz >= 0) A->p = cs_realloc (A->p, nzmax, sizeof (CSint), &okj);
    if (A->x) A->x = cs_realloc (A->x, nzmax, sizeof (CSfloat), &okx);
    ok = (oki && okj && okx);
    if (ok) A->nzmax = nzmax;
    return (ok);
}
...
csd *cs_dalloc (CSint m, CSint n)
{
    ...
    if (!D) return (NULL);
    D->P = cs_malloc (m, sizeof (CSint));
    D->R = cs_malloc (m+6, sizeof (CSint));
    D->Q = cs_malloc (n, sizeof (CSint));
    D->S = cs_malloc (n+6, sizeof (CSint));
...
/* free workspace and return CSint array result */
